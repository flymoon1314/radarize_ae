...
from radarize.utils import dsp, grid_map, image_tools, radar_config
        ...
        heatmap = np.stack(
            [
                dsp.compute_range_azimuth(
                    radar_cube_a, angle_res, angle_range, "capon"
                ),
            ]
        )
                ...
                [
                    ...
                    np.rot90(
                        ...
                        image_tools.polar2cartesian(
                            ...
                            np.linspace(0.0, range_max, x.shape[0]),
                            np.arange(
...
def create_camera_fpv(bag, camera_topic="/tracking/fisheye1/image_raw/compressed"):
    """Extract camera images from bag file."""

    camera_ts, camera_msgs = [], []

    for i, (topic, msg, ts) in tqdm(
        enumerate(bag.read_messages([camera_topic])),
...
def create_depth_bev(
    ...
    def depth2pcd(depth, intrinsic, depth_trunc=3.0):
        ...
        pcd = o3d.geometry.PointCloud.create_from_depth_image(
            depth_img, intrinsic, depth_trunc=depth_trunc
        )
        return np.asarray(pcd.points)

    for i, (topic, msg, ts) in tqdm(
        enumerate(bag.read_messages([depth_topic])),
        ...
        if warp_cartesian:
            depthmap = grid_map.generate_ray_casting_grid_map(
                pcd, range_max, range_bins
            )
        else:
            depthmap = grid_map.generate_ray_casting_polar_map(
                pcd,
                np.linspace(
                    0,
                    range_max,
...
def create_pcd_bev(
    ...
    for i, (topic, msg, ts) in tqdm(
        enumerate(bag.read_messages([pcd_topic])),
        ...
        if warp_cartesian:
            ...
            pcdmap = grid_map.generate_ray_casting_polar_map(
                pcd,
                np.linspace(0, range_max, range_bins),
                np.linspace(
                    np.deg2rad(-angle_range),
                    np.deg2rad(angle_range),
                    angle_bins,
...
def create_imu(bag, imu_topic="/tracking/imu"):
    imu_ts, imu_msgs = [], []

    for i, (topic, msg, ts) in tqdm(
        enumerate(bag.read_messages([imu_topic])),
...
def create_body_velo_gt(bag, pose_topic="/tracking/odom/sample"):
    ...
    for topic, msg, ts in tqdm(
        ...
        if last_ts is None:
            ...
            if (
                body_velo_gt_msgs
                and np.linalg.norm(body_velo - body_velo_gt_msgs[-1]) > 0.5
            ):
...
def create_pose_gt(bag, pose_topic="/tracking/odom/sample"):
...
if __name__ == "__main__":
    ...
    # DA heatmaps
    da_ts, da_heatmaps = create_radar_doppler(
        bag,
        radar_params,
    ...
    da_e_ts, da_e_heatmaps = create_radar_doppler_elevation(
        bag,
        radar_params,
        radar_topic=cfg["RADAR_TOPIC"],
        resize_shape=cfg["DA"]["RESIZE_SHAPE"],
    ...
    ra_1_ts, ra_1_heatmaps = create_radar_bev(
        bag,
        radar_params,
        radar_topic=cfg["RADAR_TOPIC"],
        radar_buffer_len=1,
    ...
    ra_3_ts, ra_3_heatmaps = create_radar_bev(
        bag,
        radar_params,
        radar_topic=cfg["RADAR_TOPIC"],
        radar_buffer_len=3,
    ...
    ra_5_ts, ra_5_heatmaps = create_radar_bev(
        bag,
        radar_params,
        radar_topic=cfg["RADAR_TOPIC"],
        radar_buffer_len=5,
    ...
    ra_e_1_ts, ra_e_1_heatmaps = create_radar_bev_elevation(
        bag,
        radar_params,
        radar_topic=cfg["RADAR_TOPIC"],
        radar_buffer_len=1,
    ...
    ra_e_3_ts, ra_e_3_heatmaps = create_radar_bev_elevation(
        bag,
        radar_params,
        radar_topic=cfg["RADAR_TOPIC"],
        radar_buffer_len=3,
    ...
    ra_e_5_ts, ra_e_5_heatmaps = create_radar_bev_elevation(
        bag,
        radar_params,
        radar_topic=cfg["RADAR_TOPIC"],
        radar_buffer_len=5,
    ...
    # Depth maps.
    depth_ts, depth_maps = create_depth_bev(
        bag, depth_intrinsics, radar_params, depth_topic=cfg["DEPTH_TOPIC"]
    )

    depth_u_ts, depth_maps_u = create_depth_bev(
        bag,
        depth_intrinsics,
        radar_params,
    ...
    # Point clouds.
    pcd_ts, pcd_maps = create_pcd_bev(bag, radar_params, pcd_topic=cfg["PCD_TOPIC"])
    ...
    # IMU data.
    imu_ts, imu_msgs = create_imu(bag, imu_topic=cfg["IMU_TOPIC"])

    # Calculate GT
    pose_ts, pose_msgs = create_pose_gt(bag, pose_topic=cfg["POSE_TOPIC"])

    body_velo_ts, body_velo_msgs = create_body_velo_gt(
        bag, pose_topic=cfg["POSE_TOPIC"]
    )
